...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const openQna = async (qnaNo) => {
    if (!qnaNo) return;
    const { ok, data } = await fetchJson(`/admin/api/qna/${qnaNo}`);
    if (!ok) {
  ...
  const saveReply = async () => {
    ...
    const { ok, data } = await fetchJson(`/admin/api/qna/${currentQnaNo}/reply`, {
      method: 'POST',
      headers: {
  ...
  const deleteQna = async () => {
    ...
    const { ok, data } = await fetchJson(`/admin/api/qna/${currentQnaNo}`, { method: 'DELETE' });
