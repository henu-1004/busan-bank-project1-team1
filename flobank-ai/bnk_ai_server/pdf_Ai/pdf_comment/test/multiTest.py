...
# 6) 청크 기반 멀티 분석
# -------------------------------------------------------
def analyze_chunk(chunk, retriever, rules_per_phrase=10, rerank_top_k=5):
    risky_list = extract_risky_phrases(chunk)

    # 위험문구가 없다면 청크 전체 1회 분석
    if len(risky_list) == 0:
        risky_list = [chunk]
    ...
    for phrase in risky_list:
        retrieved = retriever.invoke(phrase)[:rules_per_phrase]
        reranked = rerank_rules(phrase, retrieved)
        selected_rules = "\n\n".join(reranked[:rerank_top_k])

        violation_json = check_violation(phrase, chunk, selected_rules)
...
def analyze(lines, retriever):
    ...
    for idx, chunk in enumerate(chunks):
        ...
        chunk_result = analyze_chunk(chunk, retriever)
...
if __name__ == "__main__":
    ...
    target_txt = "/home/g5223sho/bnk_ai_server/pdf_Ai/pdf_comment/pdf_temp/0b8f70d8-f9cd-49be-9719-221fd5f203b9_FLOBANK 외화수퍼플러스 상품설명서.txt"

    lines = load_lines(target_txt)
    result = analyze(lines, retriever)

    os.makedirs("analysis_result", exist_ok=True)
    json.dump(result, open("analysis_result/detail.json", "w", encoding="utf-8"),
              ensure_ascii=False, indent=2)

    final_comment = generate_ai_comment(result)
    open("analysis_result/final_comment.json", "w", encoding="utf-8").write(final_comment["llm_comment"])

    print("\n🏁 분석 완료")
    print("🔢 Danger 개수:", final_comment["danger_count"])
    print("🏷 최종 등급:", final_comment["overall"])
